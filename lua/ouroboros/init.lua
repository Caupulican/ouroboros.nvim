...
function M.switch()
		...
		for _, p in ipairs({ path, "." }) do
			...
			if next(matching_files) ~= nil then
				for _, file_path in ipairs(matching_files) do
					...
					local score =
						utils.calculate_final_score(current_file, file_path, current_file_extension, file_extension)
	...
	if found_match then
		...
		vim.ui.input(
			{ prompt = "Failed to find a matching file, would you like to create at: ", default = could_create_at },
			function(input)
				if input == nil then
					return false
				else
