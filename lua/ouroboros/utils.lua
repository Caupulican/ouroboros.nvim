...
function M.edit(filepath)
	...
	-- Step 1: Try to switch to file if it's open in any visible window
	if M.switch_to_open_file_if_possible(abs_path) then
		return
	end

	-- Step 2: Search for a loaded buffer in the same directory
	for _, buf in ipairs(vim.api.nvim_list_bufs()) do
		if vim.api.nvim_buf_is_loaded(buf) then
			...
			if buf_path == abs_path and buf_dir == caller_dir then
